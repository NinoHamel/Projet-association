...
<?php
    ...
    if ($_SESSION['role'] == 'Admin')           // si la personne connectée est bien un admin 
    {
        ...
        {
            $pseudo=$_GET['pseudo'];
            $promouvoir = $db -> prepare("UPDATE utilisateurs SET roleutilisateur= 'Tresorier' WHERE pseudo=:pseudo");// rendre trésorier la personne 
            $promouvoir-> execute(['pseudo' => $pseudo]);                                                 // dont le pseudo est celui dans la barre de recherche

            echo " <br/> Le compte associé au pseudo " . $pseudo . " a bien été promu trésorier ";
